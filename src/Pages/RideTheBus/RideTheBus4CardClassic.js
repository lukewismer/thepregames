    ...
    <div className={styles.rideTheBus}>
      ...
      <div>
        ...
        <div className={styles.btnRow}>
          {!rows[0].done ? (
            <div>
              <button className={rows[0].isButtonDark.red ? styles.redBtnDark : styles.redBtn} onClick={() => toggleButtonState(0, "red")}>RED</button>
              <button className={rows[0].isButtonDark.black ? styles.blackBtnDark : styles.blackBtn} onClick={() => toggleButtonState(0, "black")}>BLACK</button>
            </div>
          ) : !rows[1].done ? (
            <div>
              <button className={rows[1].isButtonDark.higher ? styles.greenBtnDark : styles.greenBtn} onClick={() => toggleButtonState(1, "higher")}>HIGHER</button>
              <button className={rows[1].isButtonDark.lower ? styles.redBtnDark : styles.redBtn} onClick={() => toggleButtonState(1, "lower")}>LOWER</button>
            </div>
          ) : !rows[2].done ? (
            <div>
              <button className={rows[2].isButtonDark.inside ? styles.greenBtnDark : styles.greenBtn} onClick={() => toggleButtonState(2, "inside")}>INSIDE</button>
              <button className={rows[2].isButtonDark.outside ? styles.redBtnDark : styles.redBtn} onClick={() => toggleButtonState(2, "outside")}>OUTSIDE</button>
            </div>
          ) : !rows[3].done ? (
            <div>
              <button className={rows[3].isButtonDark.hearts ? styles.redBtnDark : styles.redBtn} onClick={() => toggleButtonState(3, "hearts")}>HEARTS</button>
              <button className={rows[3].isButtonDark.diamonds ? styles.redBtnDark : styles.redBtn} onClick={() => toggleButtonState(3, "diamonds")}>DIAMONDS</button>
              <button className={rows[3].isButtonDark.clubs ? styles.blackBtnDark : styles.blackBtn} onClick={() => toggleButtonState(3, "clubs")}>CLUBS</button>
              <button className={rows[3].isButtonDark.spades ? styles.blackBtnDark : styles.blackBtn} onClick={() => toggleButtonState(3, "spades")}>SPADES</button>
            </div>
          ) : null}
        </div>
        ...
        <button className={styles.shuffleBtn} onClick={handleShuffle}>Restart</button>
          ...
          {rows.map((row, index) => (
            row.card ? (
              <Card
                ...
                suit={row.card.suit}
                faceUp={true}
                className={row.done ? `${styles.card} ${styles.done}` : `${styles.card} ${styles.wrong}`}
              />
            ) : (
